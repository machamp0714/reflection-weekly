...
- [x] 2.1 (P) GitHubクライアントを実装する
  ...
  - 指定期間のPR（Pull Request）取得機能を実装する
  - PRタイトル、PR説明文（description）、作成日時、リポジトリ名の抽出を実装する
  - 指定期間内の作成PR数の集計をサポートする
  - 複数リポジトリからのPR取得をサポートする
  - ページネーション処理を実装する
...
- [x] 3.1 データ統合機能を実装する
  ...
  - 両データソースの時系列統合を実装する
  - 日別の活動サマリー集計を実装する
  - プロジェクト別の活動量（PR数、作業時間）集計を実装する
  - いずれかのデータソースが空の場合の継続処理を実装する
  - 部分的データ取得失敗時の警告生成とグレースフル継続を実装する
  - 2.1と2.2に依存
  - _Requirements: 1.3, 1.5, 2.4, 2.5, 3.1, 3.5, 3.7_

- [x] 3.2 活動分析機能を実装する
  - OpenAI APIを使用した日別活動サマリー生成を実装する
  - PR内容と作業時間からの主要成果・活動の自然言語要約を実装する
  - PR数と作業時間の相関分析・洞察生成を実装する
...
- [x] 5. Presentation層のCLIを実装する
- [x] 5.1 メイン振り返りコマンドを実装する
...
- [x] 6. スケジュール実行機能を実装する
- [x] 6.1 プラットフォーム固有のスケジューラ設定生成を実装する
...
- [x] 7. 統合テストとエンドツーエンドテストを実装する
- [x] 7.1 (P) 各クライアントのユニットテストを実装する
  - ConfigManagerの設定読込・検証・デフォルト値適用テストを実装する
  - GitHubClientのPR取得・エラーハンドリングテストを実装する
  ...
  - _Requirements: 1.4, 2.3, 3.6, 4.5, 7.10_
