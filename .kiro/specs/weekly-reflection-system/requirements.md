...
## Introduction
本ドキュメントは、週次振り返りシステムの要件を定義します。このシステムは、GitHubのコミット履歴とTogglの打刻データを統合し、Notionに自動的に振り返りページを作成することで、KPT（Keep/Problem/Try）形式の振り返りと自己成長の追跡を支援します。

## Requirements

### Requirement 1: GitHubコミットデータ取得
**Objective:** As a ユーザー, I want 1週間分のGitHubコミット履歴を自動取得したい, so that 振り返りの材料として活動内容を把握できる

#### Acceptance Criteria
1. When ユーザーが振り返り生成を実行した時, the システム shall 指定された期間（デフォルト: 過去7日間）のGitHubコミット履歴を取得する
2. When コミットデータを取得した時, the システム shall コミットメッセージ、日時、リポジトリ名、変更ファイル数を抽出する
3. If GitHub APIへの接続に失敗した場合, the システム shall エラーメッセージを表示し、リトライオプションを提供する
4. If 指定期間にコミットが存在しない場合, the システム shall 「該当期間のコミットはありません」と通知する
5. The システム shall 複数のリポジトリからコミット履歴を取得できる
...
3. When 活動サマリーを生成する時, the システム shall コミット内容と作業時間から主要な成果・活動を自然言語で要約する
4. The システム shall ChatGPTにコミット数と作業時間の相関分析を依頼し、洞察を生成する
5. The システム shall プロジェクト別の活動量（コミット数、作業時間）を集計する
...
3. When ページを作成する時, the システム shall GitHubコミットサマリーセクションを生成する
